...
import { validateVaultPath, toVaultPath, joinVaultPath, existsInVault } from './vault-path';
...
describe('vault-path', () => {
  ...
  describe('joinVaultPath', () => {
    ...
    it('should throw on traversal in segments', () => {
      ...
      expect(() => joinVaultPath(vaultRoot, 'notes', '..', '..', 'etc', 'passwd')).toThrow(
        'Path traversal detected'
      );
    });
    ...
    it('should handle segments with spaces', () => {
      ...
      const result = joinVaultPath(vaultRoot, 'My Notes', 'Daily Journal', 'entry.md');
  ...
  describe('existsInVault', () => {
    ...
    it('should return false for non-existing nested path', async () => {
      const result = await existsInVault(testVaultRoot, 'nonexistent/path/file.md');
    ...
    it('should return false for vault root directory (Bun.file checks files, not dirs)', async () => {
    ...
    it('should return false for subdirectory (Bun.file checks files, not dirs)', async () => {
